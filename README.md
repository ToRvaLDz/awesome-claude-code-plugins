...
- [trailhead](https://github.com/ToRvaLDz/trailhead) - Run a large project as a map of decision tickets on GitHub Issues, resolved one at a time by a discuss → plan → execute → verify engine with atomic commits; plans get an optional cross-AI review from other AI CLIs, and UI tickets start from a throwaway prototype. Self-contained, runs on Claude Code and Codex CLI.
